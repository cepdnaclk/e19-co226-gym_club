<table>
    ...
    $sql = "SELECT uf.id, uf.name, uf.gender, fg.goal_type
               ...
               WHERE user_type = 'user' AND tr.trainer_id =$u_id";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            echo "<tr class='data_row'>";
            echo "<td><span class='a_rm_trainee' data-trainer-id='" . $u_id . "' data-trainee-id='" . $row["id"] . "'>" . $row["name"] . "</span></td>";
            echo "<td>" . $row["gender"] . "</td>";
            echo "<td>" . $row["goal_type"] . "</td>";
            echo "</tr>";
        }
    } else {
        echo "<td colspan='3' class='warn'>You haven't added any Trainees yet</td>";
    }
    ?>
</table>